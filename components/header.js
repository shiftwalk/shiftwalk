      ...
      <Grid className={`items-center border-b ${textColor ? `border-grey` : 'border-black'} py-3`}>
        <div className="col-span-3">
          <div className="overflow-hidden relative">
            <Link href="/">
              <a className={`block w-24 md:w-28 xl:w-32 a11y-focus relative group motion-safe:transition-transform motion-safe:ease-[cubic-bezier(0.65,0,0.35,1)] motion-safe:duration-[500ms] ${!headerContext ? 'translate-y-full' : 'translate-y-0'} ${ shouldTransition && 'nav-item-hover' }`} onMouseEnter={handleHover } onMouseLeave={handleHoverOut } aria-label="Navigate back to the home page">
