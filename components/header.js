    ...
    <header className={`px-5 fixed top-0 left-0 right-0 z-20 ${bgColor ? `bg-blue` : 'bg-white'} ${textColor && 'text-grey' }`}>
      <Grid className={`items-center border-b ${textColor ? `border-grey` : 'border-black'} py-3`}>
        <div className="col-span-3">
          <Link href="/">
            <a className="block w-24 md:w-28 xl:w-32 a11y-focus relative group">
        ...
        <nav className="col-span-2 col-start-9 text-right text-lg md:text-xl xl:text-2xl leading-none">
          <div className="hidden md:flex space-x-[5px] justify-end items-center">
            <Link href="/">
              <a className={`a11y-focus w-auto transition-opacity ease-in-out duration-300 ${router.asPath == '/' ? 'text-black' : 'opacity-30 hover:text-black focus:text-black hover:opacity-100 focus:opacity-100' }`}>
                <span className="block">Studio,</span>
              </a>
            </Link>

            <Link href="/projects">
              <a className={`a11y-focus w-auto transition-opacity ease-in-out duration-300 ${router.asPath.includes('/projects') ? 'text-black' : 'opacity-30 hover:text-black focus:text-black hover:opacity-100 focus:opacity-100' }`}>
                <span className="block">Projects,</span>
              </a>
            </Link>

            <Link href="/journal">
              <a className={`a11y-focus w-auto transition-opacity ease-in-out duration-300 ${router.asPath.includes('/journal') ? 'text-black' : 'opacity-30 hover:text-black focus:text-black hover:opacity-100 focus:opacity-100' }`}>
                <span className="block">Journal,</span>
              </a>
            </Link>

            <Link href="/contact">
              <a className={`a11y-focus w-auto transition-opacity ease-in-out duration-300 ${router.asPath == '/contact' ? 'text-black' : 'opacity-30 hover:text-black focus:text-black hover:opacity-100 focus:opacity-100' }`}>
