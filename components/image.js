...
export default function Image({ image, layout, widthOverride, heightOverride, focalPoint, className, priority, noCaption, noBg, sizes, nonRelative, introDelay, deepGrain, quality }) {
  ...
  if (sizes) { attributes.sizes = sizes }
    ...
    <div className={`image bg-grey bg-opacity-30 relative overflow-hidden ${className} w-full ${layout == 'fill' && 'cover-image' }`}>
      ...
      <div className={`absolute inset-0 w-full h-full z-[20] transition-opacity ease-[cubic-bezier(0.65,0,0.35,1)] duration-[330ms] ${introDelay ? 'delay-[330ms]' : '' } ${imageIsLoaded ? 'opacity-0' : 'opacity-100' }`}>
      ...
      <video loop={true} autoPlay="autoplay" playsInline={true} muted className={`object-cover object-center w-full h-full absolute inset-0 z-[10] transition ease-[cubic-bezier(0.65,0,0.35,1)] duration-[330ms] ${imageIsLoaded ? 'opacity-100' : 'opacity-0' }`}>
        <source src={ image.overrideVimeoVideo ? image.overrideVimeoVideo : image.videoOverride.asset.url } type="video/mp4" />

        Sorry. Your browser does not support the video tag.
      </video>
      
      <div className={`z-[10] transition-opacity ease-[cubic-bezier(0.65,0,0.35,1)] duration-[330ms] ${imageIsLoaded ? 'opacity-100' : 'opacity-0' }`}>
        <Img
          {...imageProps}
          {...attributes}
          onLoad={event => {
